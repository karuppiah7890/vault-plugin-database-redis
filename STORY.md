...
```bash
vault-plugin-database-redis $ vault token create
Key                  Value
---                  -----
token                s.M1JxGaWrm7j7Y4kBheMwoSJn
token_accessor       ILdQF4yObkszAESaEjv2knz5
token_duration       ∞
token_renewable      false
token_policies       ["root"]
identity_policies    []
policies             ["root"]
vault-plugin-database-redis $ vault login s.M1JxGaWrm7j7Y4kBheMwoSJn
...
vault-plugin-database-redis $ vault token revoke s.M1JxGaWrm7j7Y4kBheMwoSJn
Success! Revoked token (if it existed)
vault-plugin-database-redis $ vault token revoke s.M1JxGaWrm7j7Y4kBheMwoSJn
Success! Revoked token (if it existed)
vault-plugin-database-redis $ vault auth list
Path      Type     Accessor               Description
----      ----     --------               -----------
token/    token    auth_token_fa3ebb39    token based credentials
vault-plugin-database-redis $ 
```
...
----      ----     --------               -----------
token/    token    auth_token_fa3ebb39    token based credentials
vault-plugin-database-redis $ vault policy read default
...
vault-plugin-database-redis $ vault policy read root
